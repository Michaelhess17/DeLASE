...
def simulate_single_oscillator(
    ...
    solution = diffrax.diffeqsolve(
        ...
        t1=t_span[1],
        dt0=dt,
        stepsize_controller=diffrax.PIDController(rtol=1e-6, atol=1e-8, error_order=1.5),
...
def main():
    ...
    plt.savefig(figure_path.joinpath("oscillators.png"))
    ...
    plt.legend()
    plt.show()
    plt.savefig(figure_path.joinpath("VDP_FMs.png"))

    output_path = project_path.joinpath("outputs")
    output_path.mkdir(exist_ok=True)
    jnp.save(output_path.joinpath("VDP_oscillators.npy"), solutions)
    jnp.save(output_path.joinpath("VDP_FMs.npy"), FMs)
    return
